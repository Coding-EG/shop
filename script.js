...
<p class="product-price">&#8377; ${price}</p><div class="product-quantity-container"><i class="product-minus-button"onclick="itemsDecreasement('${id}')">&minus;</i>
      ...
      searchData === undefined ? 0 : searchData.item
    }</span>
...
function navToggleFunction() {
  ...
  if (window.innerWidth <= 575) {
    ...
    navLi.forEach((x) => {
      x.classList.add("nav-li-toggle");
    });
    navHref.forEach((x) => {
      x.classList.add("nav-target-toggle");
    });
    navUl.style.flexDirection = "column";
  } else {
    navToggle.style.display = "none";
    navLi.forEach((x) => {
      x.classList.remove("nav-li-toggle");
    });
    navHref.forEach((x) => {
      x.classList.remove("nav-target-toggle");
    });
